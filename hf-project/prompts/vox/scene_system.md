...
- 🔴 **底部字幕安全区铁律：画面底部 130px 是烧录字幕的专属区域，禁止放任何内容元素**（pill 标签、卡片、说明文字、数据条都不得进入底部 130px）。所有 pill 标签/底部说明行必须放在底部 130px 之上，否则会和烧录的字幕重叠，造成"杂乱文字一闪而过"。
...
- **hero_typography**：Vox 标志性——超大粗体无衬线字（140-200px，font-weight:900）逐个字砸上屏（SLAM，scale:2.5→1 + blur 消散），一道青蓝高亮笔划（id="highlight-sweep"）横向扫过关键词下划线。底部可压 1-2 个小数据标签。**这是"看这个"的视觉锚点**，要占满画面、冲击力拉满。
