...
🔴 **铁律：每场景 1-3 个知识/数据元素。** 进度条/掌握度数字/关键公式/步骤计数/对比数值——纯文字卡片不够。

- 卡片揭示：3-4 张知识卡片依次滑入，每张含标题+要点+数字/进度
- 对比框：左右卡片 + 差异标注 + 对比数据
- 例句展示：大字号英文 + 中文翻译 + 结构标注
- 步骤流程：编号圆圈 + 箭头连线 + 每步计数
- 关键词爆炸：核心词汇居中放大 + 释义 + 使用频率数字
- 总结网格：2×2 卡片网格，每格含数字指标
