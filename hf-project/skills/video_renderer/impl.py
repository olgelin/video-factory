...
# ============================================================
# 编码容错层 — 防止 HyperFrames Node.js 在 Windows 上
# 因文件编码问题渲染失败（UnicodeDecodeError: 0xb2 in position 14）
# ============================================================

def _safe_read_html(path: Path) -> str:
    """安全读取 HTML。UTF-8 优先，失败用 errors='replace' 兜底。"""
    raw = path.read_bytes()
...
def _safe_write_html(path: Path, content: str):
    """安全写入 HTML。UTF-8 无 BOM，CRLF 转 LF。"""
    normalized = content.replace('\r\n', '\n').replace('\r', '\n')
    path.write_text(normalized, encoding="utf-8")


def _normalize_html_encoding(project_dir: str):
    """预检：重新编码所有 HTML 为纯净 UTF-8。"""
    comp_dir = Path(project_dir) / "compositions"
    if not comp_dir.exists():
        return
    fixed = 0
    for html_file in sorted(comp_dir.glob("*.html")):
        try:
            content = _safe_read_html(html_file)
            _safe_write_html(html_file, content)
            fixed += 1
        except Exception as e:
            print(f"  [video-renderer] 编码规范化失败 {html_file.name}: {e}")
    if fixed > 0:
        print(f"  [video-renderer] 编码规范化: {fixed} 个文件")
...
def _simplify_composition_gsap(project_dir: str, clip_src: str) -> bool:
    ...
    html = _safe_read_html(comp_path)
    original = html
    ...
    if html != original:
        _safe_write_html(comp_path, html)
...
def run_hyperframes_render(project_dir: str, output_path: str, supersample: bool = False) -> bool:
    ...
    index_content = _safe_read_html(index_path)
    ...
    for i, (comp_id, clip_src, duration) in enumerate(clips):
        ...
        try:
            ...
            if result.returncode == 0 and Path(clip_path).exists():
                ...
                # 重试1: 简化GSAP
                print(f"  [video-renderer]   {clip_src} 渲染失败(rc={result.returncode})，尝试简化GSAP后重试...")
            ...
            print(f"  [video-renderer]   {clip_src} 渲染超时（600s），使用空白帧兜底...")
            fallback_clip = _render_fallback_frame(temp_dir, i, duration, project_dir)
            if fallback_clip:
                segment_files.append(fallback_clip)
                print(f"  [video-renderer]   {clip_src} 使用空白帧替代")
    ...
    try:
        result = subprocess.run(
            concat_cmd, shell=True, capture_output=True,
...
def _validate_html_structure(project_dir: str) -> bool:
    ...
    comp_dir = Path(project_dir) / "compositions"
    if not comp_dir.exists():
        print(f"  [video-renderer] compositions 目录不存在")
        return False
    ...
    for html_file in sorted(comp_dir.glob("beat-*.html")):
        ...
        for ok, msg in checks:
            if not ok:
                print(f"  [video-renderer] {html_file.name}: {msg}")
                issues += 1

    if issues > 0:
        print(f"  [video-renderer] 发现 {issues} 个结构问题，继续渲染...")
    else:
...
def run(context: dict) -> dict:
    ...
    # 预检0: 编码规范化（防止 UnicodeDecodeError）
    _normalize_html_encoding(project_dir)

    # 预检1: 修复HTML中的重复style属性
