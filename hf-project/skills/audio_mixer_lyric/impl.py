...
def run(context: dict) -> dict:
    ...
    if not os.path.exists(bgm_path):
        ...
        # 合并视频+配音
        cmd = f'ffmpeg -y -i "{video_path}" -i "{voice_path}" -c:v copy -c:a aac -b:a 320k -map 0:v:0 -map 1:a:0 "{MIXED_PATH}"'
        ...
        # 再合并视频+混合音频
        cmd = f'ffmpeg -y -i "{video_path}" -i "{mixed_audio}" -c:v copy -c:a aac -b:a 320k -map 0:v:0 -map 1:a:0 "{MIXED_PATH}"'
