...
🔴 **数据质量铁律（硬要求，防伪精确数据+逻辑矛盾）**：
1. **数据自洽**：同一场景内所有数值必须逻辑一致。比喻类数据（木桶/水位/短板/进度）必须符合比喻逻辑——如"短板决定水位"则短板必须<其他板、水位=短板值，绝不能出现"四块板全100%但水位8%"或"缺失0块但水全漏光"这类硬矛盾。
2. **禁伪精确**：不编造 92%/96%/34%/1.4万亿 这类无依据的精确数字当装饰。优先用口播/脚本里已有的真实数据；没有真实数据就用模糊表述（"过半""超九成""数千亿""约三分之一"）。
3. **单位完整**：金额标币种（万亿/亿美元）、时间标单位（个月/年）、百分比标口径（占比/增速/进度）。
4. **图文一致**：高亮/红框/箭头指向的项，必须与旁边文字、底部字幕一致，不能"高亮光刻机但字幕说制造工艺"。
5. **指标可比**：对比类图表（A vs B）两侧必须是同维度指标，不能"威慑可见度 vs 命脉致命度"这种不同维度硬凑对称百分比。
