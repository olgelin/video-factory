...
⚠️ **致命规则**：你生成的内容会直接被注入到已有的HTML外壳中。**绝对禁止**输出 `<!DOCTYPE html>`、`<html>`、`<head>`、`<body>` 标签。只输出 `<div class="scene">` + `<script>` 两个块，不要外层包裹任何东西。**禁止输出任何 HTML 之外的文字**——不要输出"视觉解析""数据呈现""优化建议"等 Markdown 说明、注释、总结、提示，`</script>` 之后必须立即结束，一个多余的字都不能有。
...
- 🔴 ghost text 水印（必须有！）：中文关键词，140-200px，透明度 3-6%，z-index 低于内容。**关键词必须与前景主标题/主标签不同**（不能前景"时间差"、水印也"时间差"，造成视觉重复）
...
🔴 **铁律：每场景 1-3 个数据元素，硬要求。** quote_hero/timeline_event/compare 最容易漏——但必须有 KPI 卡片/进度条/趋势指标。**数据元素可以是文字形式**（进度条+文字标签、趋势箭头+文字、KPI 卡片+文字值），当 key_elements/narration 没有真实数字时，用文字或模糊表述（"过半""持续走低""倒数第一"）填数据位，**绝不编造精确数字**（-37%/-52% 这类）来凑"数据可视化"。
...
2. **禁伪精确**：不编造 92%/96%/34%/1.4万亿 这类无依据的精确数字当装饰。**数据只能来自 key_elements/narration 里已有的真实数据**；key_elements 给的是文字（如"越存越亏"）就用文字，绝不自己编造 -37%/+147% 这类 key_elements 里没有的数字。没有真实数据就用模糊表述（"过半""超九成""数千亿""约三分之一"）。
...
6. **禁占位符/空值**：禁止 `$0.0T`、`0`、`NaN`、`""`、`xxx`、`--` 这类占位符或空值。每个数据元素必须是真实、有意义的数值，宁可用模糊表述（"数千亿"）也不留空或写 0。
7. **图形比例匹配数值**：柱状图/对比条/进度条的长度、高度必须与数值成正比。-37% 与 +147% 的长度比应约 1:4，绝不能画成 1:1。
8. **对比守恒**：此消彼长/转移类对比数据必须逻辑守恒或对称（损失方 ≈ 获益方，或明确标注口径），不能出现"储蓄者 -37% + 借贷者 +147% = +110%"这种不守恒。
