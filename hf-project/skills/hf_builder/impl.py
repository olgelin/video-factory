...
10. 字体: 'Inter','Noto Sans SC','Source Han Sans SC',sans-serif | 数据: 'JetBrains Mono',monospace | 行高:1.6 | letter-spacing:0.02em
...
- **数字冲击动画**: 核心数据出现时必须有"冲击效果"——数字从远处飞入(scale:2→1)+发光脉动(text-shadow强度0→50px→30px)+周围粒子扩散。这是画面高级感的核心！
...
三层之间必须有明确的z-index区分，不能混在一起。内容层的元素必须在装饰层之上可读。

## 色彩饱和度要求（禁止灰蒙蒙）
- 主色饱和度必须≥60%（HSL的S值）。如果design.md的主色太灰，自行提升饱和度
- 核心数据必须用高亮色（金色#FFD700/青色#00D4FF/紫色#A855F7），不能用灰色
- 卡片边框必须有颜色（rgba(主色, 0.3)），不能用白色/灰色边框
- 每个场景至少有2种不同颜色（主色+强调色），不能只用一种颜色的不同明暗
...
- 入场无节奏 → 必须0.15s交错
- 灰蒙蒙的画面 → 核心数据必须高亮色，不能灰色，主色饱和度≥60%
- 数据出现无冲击 → 核心数字必须有scale+glow冲击效果
- 只有一层结构 → 必须背景层+内容层+装饰层三层分明
- 无数据可视化 → 有数字的场景必须有countUp动画+数据卡片
