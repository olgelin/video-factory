...
## 图表模板（当 chart_type 不为 null 时使用）
### bar_chart（柱状图）
- 5-7根柱子，每根带标签+数值，用CSS div实现
- 柱子从0生长到目标高度（GSAP scaleY:0→1, transformOrigin:bottom）
- 柱子间距均匀，颜色渐变（主色→强调色）
- 示例: `<div style="width:80px;height:{value}%;background:linear-gradient(180deg,{primary},{accent});border-radius:4px 4px 0 0;"></div>`
### line_chart（折线图）
- SVG polyline 连接数据点，stroke-dasharray动画绘制
- 数据点用小圆点标记
- 底部X轴标签，左侧Y轴刻度
### pie_chart（饼图）
- SVG circle + stroke-dasharray 实现扇形
- 最多6片，每片不同颜色
- 中心放总计数字
### kpi_grid（指标卡片）
- 3-4个卡片并排，每个含：标签(小字灰色)+数值(大字金色)+趋势箭头

## 镜头运动 → GSAP 转化（camera_motion 不为 null 时）
- dolly_in: 整个scene从scale(1.15)缩小到scale(1.0)，duration 1.2s, ease:"power2.out"
- dolly_out: 整个scene从scale(0.9)放大到scale(1.0)，duration 1.0s, ease:"power2.out"
- pan_left: 整个scene从x:-60移动到x:0，duration 0.8s, ease:"power3.out"
- pan_right: 整个scene从x:60移动到x:0，duration 0.8s, ease:"power3.out"
- zoom_in: 核心元素从scale(0.8)放大到scale(1.0)，duration 0.6s, ease:"back.out(1.4)"
- 实现方式: tl.from(".scene", {{scale:1.15, duration:1.2, ease:"power2.out"}}, 0);
