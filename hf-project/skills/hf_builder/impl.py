...
4. **重要：不要在CSS中设置opacity:0！** 元素默认应该是可见的。用GSAP的tl.set()设置初始状态，然后用tl.from()做入场动画
...
## GSAP动画正确写法
```javascript
// 正确：用tl.set()设置初始状态，用tl.from()做动画
tl.set("#element", {opacity: 0});  // 设置初始状态
tl.from("#element", {opacity: 1, duration: 0.5}, 0);  // 动画到可见

// 错误：不要在CSS中写opacity:0
