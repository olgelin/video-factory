...
## 问题背景
1. 用户反馈歌词"很一般"，缺情绪共鸣。
2. 根因：歌词 prompt 硬编码在 impl.py（违反"prompt 独立 .md"铁律），且丢失了几个最提质的约束——"遗憾美学""日常意象代替直白情绪词""对话感"。

## 变更内容
- `lyrics_writer/impl.py`：删掉硬编码的 system_prompt + prompt，改为从独立 .md 加载（-94 行 +18 行）
- 新增 `prompts/lyrics_system.md`：system prompt，含"事件→情绪"创作逻辑（用《天路》青藏铁路→喜悦、《国歌》民族危亡→奋起 说明"事→情"映射）+ 遗憾美学 + 日常意象 + 对话感
- 新增 `prompts/lyrics_user.md`：user prompt 模板，占位符由 impl.py 用 .format() 填充

## 核心创作逻辑（新增）
事件 → 情绪：好的歌不是复述事件，而是从事件里抽出最戳人的那一种情绪。先抓情绪，再写歌。

## 验证
- 实测"789万被骗"选题生成歌词，质量明显提升：
  - 遗憾美学："若那日电话再响一声，他会不会听出那是谎"
  - 日常意象："一件六块钱的衣裳，裹着二十年攒下的光""攥着一把攥不住的沙"
  - 对话感："你问他图什么啊，他说熬到头就好了"
  - 事件→情绪：抓住"省吃俭用20年 vs 6天被骗光"的荒诞与遗憾
